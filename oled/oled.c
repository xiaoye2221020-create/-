...
uint8_t I2C_ReceiveAck(void)
{
	...
	HAL_GPIO_WritePin(OLED_SCL_GPIO, OLED_SCL_PIN, GPIO_PIN_SET);
	/*读取应答位*/
	AckBit = HAL_GPIO_ReadPin(OLED_SDA_GPIO, OLED_SDA_PIN);
	HAL_GPIO_WritePin(OLED_SCL_GPIO, OLED_SCL_PIN, GPIO_PIN_RESET);
...
/**
  ...
  * 返 回 值：无
  * 说    明：使用软件模拟I2C发送命令，控制字节为0x00
  */
void OLED_WriteCommand(uint8_t Command)
{
	I2C_Start();
	I2C_SendByte(OLED_ADDRESS);		//发送从机地址+写方向
	I2C_ReceiveAck();
	I2C_SendByte(0x00);				//控制字节，0x00表示写命令
	I2C_ReceiveAck();
	I2C_SendByte(Command);			//命令字节
	I2C_ReceiveAck();
	I2C_Stop();
}
...
void OLED_WriteData(uint8_t *Data, uint8_t Count)
{
	...
	I2C_SendByte(OLED_ADDRESS);		//发送从机地址+写方向
	I2C_ReceiveAck();
	I2C_SendByte(0x40);				//控制字节，0x40表示写数据
	I2C_ReceiveAck();
	for (i = 0; i < Count; i++)
	{
		I2C_SendByte(Data[i]);
...
/**
  * 函    数：OLED重定向printf函数，支持设置显示位置、字体大小和最大显示长度
  ...
  *                 OLED_6X8		宽6像素，高8像素
  *                 OLED_12X24		宽12像素，高24像素
  * 参    数：MaxLength 指定最大显示字符长度，范围：0~255，超出部分将被截断
  ...
  *           当字体大小为OLED_8X16时，中文字符以16*16点阵正常显示
  *           当字体大小为OLED_6X8时，中文字符以6*8点阵显示'?'
  *           当MaxLength为0时，不限制显示长度（最大256字节）
  * 说    明：调用此函数后，要想真正地呈现在屏幕上，还需调用更新函数
  */
void OLED_Printf(int16_t X, int16_t Y, uint8_t FontSize, uint16_t MaxLength, const char *format, ...)
{
	char String[256];						//定义字符数组
	char DisplayString[256];				//定义显示字符数组
	va_list arg;							//定义可变参数列表数据类型的变量arg
	va_start(arg, format);					//从format开始，接收参数列表到arg变量
	vsnprintf(String, sizeof(String), format, arg);	//使用vsnprintf打印格式化字符串到字符数组，防止溢出
	va_end(arg);							//结束变量arg
	
	/*如果设置了最大显示长度，则截断字符串*/
	if (MaxLength > 0 && MaxLength < sizeof(String))
	{
		strncpy(DisplayString, String, MaxLength);	//复制指定长度的字符串
		DisplayString[MaxLength] = '\0';			//确保字符串以'\0'结尾
	}
	else
	{
		strcpy(DisplayString, String);				//复制完整字符串
	}
	
	OLED_ShowString(X, Y, DisplayString, FontSize);	//OLED显示字符数组（字符串）
}
